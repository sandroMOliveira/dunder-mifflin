...
(s/defn one-product :- (s/maybe d.model/Product)
  ...
  (let [product (d/pull snapshot '[* {:product/category [:category/id :category/name]}] [:product/id product-id])
        product (datomic-to-entity product)]
...
(s/defn one-product!! :- (s/maybe d.model/Product)
  [snapshot, product-id :- java.util.UUID]
  (let [product (one-product snapshot product-id)]
    (when (nil? product)
...
(s/defn all-categories :- [d.model/Category]
  [snapshot]
  (datomic-to-entity (d/q '[:find [(pull ?e [*]) ...]
...
(defn create-sample-data [conn]
  ...
  (add-categories! conn [papers, printers, online])
  ...
  (pprint @(upsert-products! conn [paper-a4, butter-paper, cheap-butter-paper, epson-tx123, docs-online] "0.0.0.0"))

  (set-categories! conn [paper-a4, butter-paper, cheap-butter-paper] papers)
  (set-categories! conn [epson-tx123] printers)
  (set-categories! conn [docs-online] online))


;implicit pull
(s/defn all-products :- [d.model/Product]
  [snapshot]
  (datomic-to-entity (d/q '[:find [(pull ?entity [* {:product/category [*]}]) ...]
                            :where [?entity :product/name]] snapshot)))
...
;  [snapshot]
;  (datomic-to-entity (d/q '[:find [(pull ?product [* {:product/category [*]}]) ...]
;                            :in $ %
